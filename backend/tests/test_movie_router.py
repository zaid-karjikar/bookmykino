...
class TestGetMovies:
    ...
    def test_passes_limit_and_offset(self):
        ...
        with patch(
            "app.services.movie_service.get_all_movies", return_value=payload
        ) as mock_svc:
            client.get("/movies/?limit=10&offset=20")
            mock_svc.assert_called_once_with(
                limit=10,
                offset=20,
                playing_today_only=False,
    ...
    def test_passes_search_term(self):
        payload = {"items": [], "total": 0}
        with patch(
            "app.services.movie_service.get_all_movies", return_value=payload
        ) as mock_svc:
            client.get("/movies/?search=dune")
            _, kwargs = mock_svc.call_args
            assert kwargs["search"] == "dune"

    def test_search_accepts_punctuation_and_unicode(self):
        payload = {"items": [], "total": 0}
        with patch(
            "app.services.movie_service.get_all_movies", return_value=payload
        ) as mock_svc:
            resp = client.get(
                "/movies/", params={"search": 'Der Schuh des Manitu (2%), "x"'}
            )
            assert resp.status_code == 200
            _, kwargs = mock_svc.call_args
            assert kwargs["search"] == 'Der Schuh des Manitu (2%), "x"'

    def test_search_rejects_overlong_term(self):
        payload = {"items": [], "total": 0}
        with patch("app.services.movie_service.get_all_movies", return_value=payload):
            resp = client.get("/movies/", params={"search": "a" * 101})
...
class TestGetMovie:
    def test_returns_200_when_found(self):
        with patch(
            "app.services.movie_service.get_movie_by_id", return_value=_movie_response()
        ):
...
class TestGetShowtimes:
    def test_returns_200_with_showtime_list(self):
        with patch(
            "app.services.movie_service.get_showtimes_for_movie",
            return_value=[_showtime_response()],
        ):
    ...
    def test_returns_empty_list_when_no_showtimes(self):
        with patch(
            "app.services.movie_service.get_showtimes_for_movie", return_value=[]
        ):
            resp = client.get(f"/movies/{MOVIE_ID}/showtimes")
